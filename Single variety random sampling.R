setwd("data/RCCL")
myY=read.table("RCCL_04.txt",head=F)

golden_retriever =read.table("golden_retriever.txt",head=F)



myY<-myY[myY[,1] %in% golden_retriever[,1],]
golden_retriever<-golden_retriever[golden_retriever[,1] %in% myY[,1],]

order=match(myY[,1],golden_retriever[,1])
myY=myY[order,]


write.table(myY,paste("RCCL_05.txt",sep=''),quote=F,row.names=F,col.names = F)
...
setwd("data/RCCK")
myY=read.table("RCCL_05.txt",head=F)
...
for(r in 1:cycles){
  ...
  write.table(myY_R,paste("data/RCCLpheno_reference_rep_",i,".txt",sep=''),quote=F,row.names=F,col.names = F)
}
}
...
gen=read.table("/home/lizhuo/dog/dog/data/dog-3.txt")
...
for(i in repA:repB){
  ...
  for(j in 1:length(method)){
      ...
      myY_P[w,r]=myGS
      write.table(myY_P,paste("data/RCCL",method[j],"_pheno_prediction_rep_",i,".txt",sep=''),quote=F,row.names=F,col.names = F)
